...
vector<char> read_bytes(const string& filename) {
    ...
    if (!is) {
        throw error(format("read_bytes: filename=%s nicht lesbar",
                           filename.c_str()));
    }
    is.seekg(0, is.end); // go to end of file
    streamsize length = is.tellg(); // length/size of file
    is.seekg (0, is.beg); // go to start of file
    if (length > MAX_FILESIZE || length < 0) {
        throw error(format("read_bytes: filename=%s zu gross",
                           filename.c_str()));
    }
    vector<char> buffer(length);
    is.read(buffer.data(), length); // read completely
    if (!is.good()) {
        throw error(format("read_bytes: filename=%s Einlesefehler",
                           filename.c_str()));
    }
    is.close();
...
static std::vector<int> read_pgm5(const string& filename) {
    ...
    if (cur == -1) {
        throw error(format("read_pgm5:: filename=%s, %s",
                           filename.c_str(),
                           "keine Breite/Höhe/Helligkeit"));
    }
    ...
    if (no_bytes != needed) {
        string fmt = "filename=%s, width=%d, height=%d, maxbright=%d,\n";
            throw error(format("read_pgm5:: " + fmt +
                               "           no_bytes=%d, needed=%d",
                               filename.c_str(),
                               width, height, maxbright,
                               no_bytes, needed ));
    }
    ...
    if (twobyte) {
        for (size_t i=0; i < needed; i += 1) {
            img[i+3] = reinterpret_cast<unsigned char&>(raw[offset+2*i])*256;
            img[i+3] += reinterpret_cast<unsigned char&>(raw[offset+2*i+1]);
        }
    } else {        
        for (size_t i=0; i < needed; i += 1) {
            img[i+3] = reinterpret_cast<unsigned char&>(raw[offset+i]);
        }
    }
    return img;
}
...
std::vector<int> read_pgm(const string& filename) {
    ifstream is(filename, ifstream::binary);
    if (!is) {
        throw error(format("read_pgm: filename=%s nicht lesbar",
                           filename.c_str()));
    }    
    ...
    if (!is.good()) {
        throw error(format("read_pgm: filename=%s Einlesefehler",
                           filename.c_str()));
    }
    is.close();
    if (buffer[0] != 'P') {
        string msg = "filename=%s magic must start with 'P' not '%c'";
        throw error(format("read_pgm: " + msg, 
...
// ascii/plain pgm, P2
static void save_pgm2(const string& filename,
...
// binary pgm, P5
static void save_pgm5(const string& filename,
                      const vector<int>& img) {
    ...
    if (img[2] < 256) {
        ...
        for (auto it = img.begin()+3; it != img.end(); ++it) {
            unsigned char b1 = (unsigned char) (*it >> 8);
            unsigned char b2 = (unsigned char) (*it & 0xff);
            out << b1; // msb first
            out << b2;
        }        
    }
...
size_t argsp_t::no_pos() {
    // we may cache, but efficiency is here not important
...
bool check_sort_one(sort_function sort, int size,
                    bool timing, int verbose) {
    ...
    long sum = 0;
    for (int val : a) { sum += val; } // compute sum
    Timer timer;
    reset_swaps();
    sort(a);
    int swaps = get_swaps();
